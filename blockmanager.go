...
// blockMsg packages a ExchangeCoin block message and the peer it came from together
...
// invMsg packages a ExchangeCoin inv message and the peer it came from together
// so the block handler has access to that information.
type invMsg struct {
	inv  *wire.MsgInv
	peer *serverPeer
...
// headersMsg packages a ExchangeCoin headers message and the peer it came from
...
// txMsg packages a ExchangeCoin tx message and the peer it came from together
// so the block handler has access to that information.
type txMsg struct {
	tx   *exccutil.Tx
	peer *serverPeer
...
func (b *blockManager) checkBlockForHiddenVotes(block *exccutil.Block) {
	var votesFromBlock []*exccutil.Tx
	...
	var newVotes []*exccutil.Tx
	var oldTickets []*exccutil.Tx
	var oldRevocations []*exccutil.Tx
	oldVoteMap := make(map[chainhash.Hash]struct{},
		int(b.server.chainParams.TicketsPerBlock))
	if template != nil {
		templateBlock := exccutil.NewBlock(template.Block)
	...
	updatedTxTreeStake := make([]*exccutil.Tx, 0,
	...
	var updatedTxTreeRegular []*exccutil.Tx
...
func (b *blockManager) handleNotifyMsg(notification *blockchain.Notification) {
	...
	case blockchain.NTBlockConnected:
		...
		// below.
		txTreeRegularValid := exccutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
			exccutil.BlockValid)
...
// queue.
func (b *blockManager) QueueTx(tx *exccutil.Tx, sp *serverPeer) {
...
func (b *blockManager) QueueBlock(block *exccutil.Block, sp *serverPeer) {
...
func (b *blockManager) ProcessBlock(block *exccutil.Block, flags blockchain.BehaviorFlags) (bool, error) {
...
func (b *blockManager) ProcessTransaction(tx *exccutil.Tx, allowOrphans bool,
	rateLimit bool, allowHighFees bool) ([]*exccutil.Tx, error) {
...
// pool.
func (b *blockManager) TicketPoolValue() (exccutil.Amount, error) {
