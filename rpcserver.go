...
var (
	...
	// data.
	gbtCoinbaseAux = &exccjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &exccjson.RPCError{
	...
	ErrRPCNoWallet = &exccjson.RPCError{
		Code:    exccjson.ErrRPCNoWallet,
...
// not needed.
func rpcInternalError(errStr, context string) *exccjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return exccjson.NewRPCError(exccjson.ErrRPCInternal.Code, errStr)
}
...
func rpcInvalidError(fmtStr string, args ...interface{}) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCInvalidParameter,
...
func rpcDeserializationError(fmtStr string, args ...interface{}) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCDeserialization,
		fmt.Sprintf(fmtStr, args...))
}

// rpcRuleError is a convenience function to convert a
// rule error to an RPC error with the appropriate code set.
func rpcRuleError(fmtStr string, args ...interface{}) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCMisc,
		fmt.Sprintf(fmtStr, args...))
}
...
func rpcDuplicateTxError(fmtStr string, args ...interface{}) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCDuplicateTx,
		fmt.Sprintf(fmtStr, args...))
}
...
// context parameter is only used in the log message and may be empty if it's
// not needed.
func rpcAddressKeyError(fmtStr string, args ...interface{}) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCInvalidAddressOrKey,
		fmt.Sprintf(fmtStr, args...))
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCNoTxInfo,
...
func rpcMiscError(message string) *exccjson.RPCError {
	return exccjson.NewRPCError(exccjson.ErrRPCMisc, message)
}
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > exccutil.MaxAmount {
			return nil, rpcInvalidError("Invalid amount: 0 >= %v "+
				"> %v", amount, exccutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err := exccutil.DecodeAddress(encodedAddr)
		if err != nil {
		...
		atomic, err := exccutil.NewAmount(amount)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
				"New amount")
		}
...
func handleCreateRawSStx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.CreateRawSStxCmd)
	...
	for i, cout := range c.COuts {
		...
		addr, err := exccutil.DecodeAddress(cout.Addr)
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
		...
		pkScript, err := txscript.GenerateSStxAddrPush(addr,
			exccutil.Amount(amountsCommitted[i]), 0x0000)
		if err != nil {
			return nil, rpcInternalError(err.Error(),
		...
		// Ensure amount is in the valid range for monetary amounts.
		if cout.ChangeAmt < 0 || cout.ChangeAmt > exccutil.MaxAmount {
			return nil, rpcInvalidError("Invalid change amount: 0 "+
				"> %v > %v", cout.ChangeAmt, exccutil.MaxAmount)
		}

		// Decode the provided address.
		addr, err = exccutil.DecodeAddress(cout.ChangeAddr)
		if err != nil {
			return nil, rpcAddressKeyError("Wrong network: %v",
				addr)
		}
...
func handleCreateRawSSGenTx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.CreateRawSSGenTxCmd)
	...
	for i, ssgenPkh := range ssgenPkhs {
		...
		if ssgenCalcAmts[i] <= 0 ||
			ssgenCalcAmts[i] > exccutil.MaxAmount {
...
func handleCreateRawSSRtx(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.CreateRawSSRtxCmd)
	...
	var feeAmt exccutil.Amount
	if c.Fee != nil {
		var err error
		feeAmt, err = exccutil.NewAmount(*c.Fee)
		if err != nil {
	...
	for i, ssrtxPkh := range ssrtxPkhs {
		// Ensure amount is in the valid range for monetary amounts.
		if sstxAmts[i] <= 0 || sstxAmts[i] > exccutil.MaxAmount {
			return nil, rpcInvalidError("Invalid SSTx amount: 0 >="+
				" %v > %v", sstxAmts[i] <= 0, exccutil.MaxAmount)
		}
...
// transaction.
func createVinList(mtx *wire.MsgTx) []exccjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]exccjson.Vin, len(mtx.TxIn))
	if blockchain.IsCoinBaseTx(mtx) {
		txIn := mtx.TxIn[0]
		vinEntry := &vinList[0]
		vinEntry.Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinEntry.Sequence = txIn.Sequence
		vinEntry.AmountIn = exccutil.Amount(txIn.ValueIn).ToCoin()
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.BlockHeight = txIn.BlockHeight
		vinEntry.BlockIndex = txIn.BlockIndex
		vinEntry.ScriptSig = &exccjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []exccjson.Vout {

	txType := stake.DetermineTxType(mtx)
	voutList := make([]exccjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout exccjson.Vout
		voutSPK := &vout.ScriptPubKey
		vout.N = uint32(i)
		vout.Value = exccutil.Amount(v.Value).ToCoin()
		...
		if commitAmt != nil {
			voutSPK.CommitAmt = exccjson.Float64(commitAmt.ToCoin())
		}
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx, txHash string, blkIdx uint32, blkHeader *wire.BlockHeader, blkHash string, blkHeight int64, confirmations int64) (*exccjson.TxRawResult, error) {
	mtxHex, err := messageToHex(mtx)
	if err != nil {
		return nil, err
	}
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.DecodeRawTransactionCmd)
	...
	txReply := exccjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.DecodeScriptCmd)
	...
	p2sh, err := exccutil.NewAddressScriptHash(script, s.server.chainParams)
	if err != nil {
		return nil, rpcInternalError(err.Error(),
	...
	reply := exccjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleEstimateStakeDiff(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.EstimateStakeDiffCmd)
	...
	if c.Tickets != nil {
		...
		userEstFlt := exccutil.Amount(userEst).ToCoin()
		userEstFltPtr = &userEstFlt
	}

	return &exccjson.EstimateStakeDiffResult{
		Min:      exccutil.Amount(min).ToCoin(),
		Max:      exccutil.Amount(max).ToCoin(),
		Expected: exccutil.Amount(expected).ToCoin(),
		User:     userEstFltPtr,
	}, nil
}
...
func handleExistsAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*exccjson.ExistsAddressCmd)

	// Attempt to decode the supplied address.
	addr, err := exccutil.DecodeAddress(c.Address)
	if err != nil {
...
func handleExistsAddresses(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*exccjson.ExistsAddressesCmd)
	addresses := make([]exccutil.Address, len(c.Addresses))
	for i := range c.Addresses {
		// Attempt to decode the supplied address.
		addr, err := exccutil.DecodeAddress(c.Addresses[i])
		if err != nil {
			return nil, rpcAddressKeyError("Could not decode "+
				"address: %v", err)
		}
...
func handleExistsMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ExistsMissedTicketsCmd)

	hashes, err := exccjson.DecodeConcatenatedHashes(c.TxHashBlob)
	if err != nil {
		return nil, err
	}
...
func handleExistsExpiredTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ExistsExpiredTicketsCmd)
...
func handleExistsLiveTicket(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ExistsLiveTicketCmd)
...
func handleExistsLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ExistsLiveTicketsCmd)
...
func handleExistsMempoolTxs(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ExistsMempoolTxsCmd)
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	results := make([]*exccjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, peer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		var result exccjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = exccjson.Bool(peer.Connected())
		...
		addrs := make([]exccjson.GetAddedNodeInfoResultAddr, 0,
			len(ipList))
		for _, ip := range ipList {
			var addr exccjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]exccjson.TxRawResult, len(txns))
		for i, tx := range txns {
		...
		rawSTxns := make([]exccjson.TxRawResult, len(stxns))
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetBlockHashCmd)
	hash, err := s.chain.BlockHashByHeight(c.Index)
	if err != nil {
		return nil, &exccjson.RPCError{
			Code: exccjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetBlockHeaderCmd)
...
func handleGetBlockSubsidy(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetBlockSubsidyCmd)
...
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(bm *blockManager, useCoinbaseValue bool, submitOld *bool) (*exccjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]exccjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		resultTx := exccjson.GetBlockTemplateResultTx{
	...
	stransactions := make([]exccjson.GetBlockTemplateResultTx, 0, numSTx)
	...
	reply := exccjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *exccjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
...
func handleGetBlockTemplateProposal(s *rpcServer, request *exccjson.TemplateRequest) (interface{}, error) {
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.server.cfIndex == nil {
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCNoCFIndex,
			Message: "Compact filters must be enabled for this command",
		}
	}

	c := cmd.(*exccjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*exccjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetHeaders(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return &exccjson.GetHeadersResult{Headers: hexBlockHeaders}, nil
}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &exccjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := exccjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	result := exccjson.GetMiningInfoResult{
		Blocks:           best.Height,
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &exccjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*exccjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*exccjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.StatsSnapshot()
		info := &exccjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetRawMempoolCmd)
	...
	if c.TxType != nil {
		switch exccjson.GetRawMempoolTxTypeCmd(*c.TxType) {
		case exccjson.GRMRegular:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeRegular
		case exccjson.GRMTickets:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSStx
		case exccjson.GRMVotes:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSGen
		case exccjson.GRMRevocations:
			filterType = new(stake.TxType)
			*filterType = stake.TxTypeSSRtx
		case exccjson.GRMAll:
			// Nothing to do
		default:
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetRawTransactionCmd)
...
func handleGetStakeDifficulty(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if err != nil {
		...
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCDifficulty,
			Message: "Error getting stake difficulty: " + err.Error(),
		}
	}
	currentSdiff := exccutil.Amount(blockHeader.SBits)
	...
	nextSdiffAmount := exccutil.Amount(nextSdiff)

	sDiffResult := &exccjson.GetStakeDifficultyResult{
...
func convertVersionMap(m map[int]int) []exccjson.VersionCount {
	sorted := make([]exccjson.VersionCount, 0, len(m))
	...
	for _, v := range order {
		sorted = append(sorted, exccjson.VersionCount{Version: uint32(v),
			Count: uint32(m[v])})
	}
...
func handleGetStakeVersionInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	count := int32(1)
	c, ok := cmd.(*exccjson.GetStakeVersionInfoCmd)
	if !ok {
	...
	result := exccjson.GetStakeVersionInfoResult{
		CurrentHeight: snapshot.Height,
		Hash:          snapshot.Hash.String(),
		Intervals:     make([]exccjson.VersionInterval, 0, count),
	}
	...
	for i := int32(0); i < count; i++ {
		...
		versionInterval := exccjson.VersionInterval{
			StartHeight:  endHeight,
...
func handleGetStakeVersions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.GetStakeVersionsCmd)
	...
	result := exccjson.GetStakeVersionsResult{
		StakeVersions: make([]exccjson.StakeVersions, 0, len(sv)),
	}
	for _, v := range sv {
		nsv := exccjson.StakeVersions{
			Hash:         v.Hash.String(),
			Height:       v.Height,
			BlockVersion: v.BlockVersion,
			StakeVersion: v.StakeVersion,
			Votes: make([]exccjson.VersionBits, 0,
				len(v.Votes)),
		}
		for _, vote := range v.Votes {
			nsv.Votes = append(nsv.Votes,
				exccjson.VersionBits{Version: vote.Version,
					Bits: vote.Bits})
		}
...
func handleGetVoteInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c, ok := cmd.(*exccjson.GetVoteInfoCmd)
	if !ok {
		return nil, rpcInvalidError("Invalid type: %T", c)
	}
	...
	// Assemble JSON result.
	result := exccjson.GetVoteInfoResult{
		CurrentHeight: snapshot.Height,
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if txFromMempool != nil {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &exccjson.RPCError{
				Code: exccjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &exccjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: confirmations,
		Value:         exccutil.Amount(value).ToUnit(exccutil.AmountCoin),
		Version:       int32(txVersion),
		ScriptPubKey: exccjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
func handleGetWorkSubmission(s *rpcServer, hexData string) (interface{}, error) {
	...
	// blockchain.
	tempBlock := exccutil.NewBlockDeepCopy(blockInfo.msgBlock)
	...
	err = blockchain.CheckProofOfWork(&block.MsgBlock().Header, activeNetParams)
	if err != nil {
...
func handleLiveTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return exccjson.LiveTicketsResult{Tickets: ltString}, nil
}
...
func handleMissedTickets(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return exccjson.MissedTicketsResult{Tickets: mtString}, nil
}
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]exccjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]exccjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		amountIn := exccutil.Amount(txIn.ValueIn).ToCoin()
	...
	vinList := make([]exccjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &exccjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     exccutil.Amount(originTxOut.Value).ToCoin(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr exccutil.Address, numToSkip, numRequested uint32) ([]*exccutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*exccjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	srtList := make([]exccjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.SendRawTransactionCmd)
...
func min(s []exccutil.Amount) exccutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func max(s []exccutil.Amount) exccutil.Amount {
	max := exccutil.Amount(0)
	for i := range s {
		if s[i] > max {
			max = s[i]
		}
...
func mean(s []exccutil.Amount) exccutil.Amount {
	sum := exccutil.Amount(0)
	for i := range s {
		sum += s[i]
	}
	...
	return sum / exccutil.Amount(len(s))
}

// median gets the median amount from a slice of amounts.
func median(s []exccutil.Amount) exccutil.Amount {
	if len(s) == 0 {
		return 0
	}
...
func stdDev(s []exccutil.Amount) exccutil.Amount {
	var total float64
	mean := mean(s)
	...
	amt, _ := exccutil.NewAmount(math.Sqrt(v))
...
// memory pool.
func feeInfoForMempool(s *rpcServer, txType stake.TxType) *exccjson.FeeInfoMempool {
	txDs := s.server.txMemPool.TxDescs()
	ticketFees := make([]exccutil.Amount, 0, len(txDs))
	for _, txD := range txDs {
		if txD.Type == txType {
			feePerKb := (exccutil.Amount(txD.Fee)) * 1000 /
				exccutil.Amount(txD.Tx.MsgTx().SerializeSize())
			ticketFees = append(ticketFees, feePerKb)
		}
	}

	return &exccjson.FeeInfoMempool{
...
// properly set.
func calcFeePerKb(tx *exccutil.Tx) exccutil.Amount {
	var in exccutil.Amount
	for _, txIn := range tx.MsgTx().TxIn {
		in += exccutil.Amount(txIn.ValueIn)
	}
	var out exccutil.Amount
	for _, txOut := range tx.MsgTx().TxOut {
		out += exccutil.Amount(txOut.Value)
	}

	return ((in - out) * 1000) / exccutil.Amount(tx.MsgTx().SerializeSize())
}

// feeInfoForBlock fetches the ticket fee information for a given tx type in a
// block.
func ticketFeeInfoForBlock(s *rpcServer, height int64, txType stake.TxType) (*exccjson.FeeInfoBlock, error) {
	...
	txFees := make([]exccutil.Amount, txNum)
	itr := 0
...
// from [start, end).
func ticketFeeInfoForRange(s *rpcServer, start int64, end int64, txType stake.TxType) (*exccjson.FeeInfoWindow, error) {
	hashes, err := s.chain.HeightRange(start, end)
	if err != nil {
		return nil, err
	}

	var txFees []exccutil.Amount
	for i := range hashes {
...
func handleTicketFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.TicketFeeInfoCmd)
	...
	var feeInfoBlocks []exccjson.FeeInfoBlock
	blocks := uint32(0)
	if c.Blocks != nil {
		blocks = *c.Blocks
	}
	...
	var feeInfoWindows []exccjson.FeeInfoWindow
	windows := uint32(0)
	if c.Windows != nil {
		windows = *c.Windows
	}
	...
	return &exccjson.TicketFeeInfoResult{
...
func handleTicketsForAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.TicketsForAddressCmd)
	...
	reply := &exccjson.TicketsForAddressResult{
...
func handleTicketVWAP(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	return exccutil.Amount(vwap).ToCoin(), nil
}
...
func handleTxFeeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	var feeInfoRange exccjson.FeeInfoRange
	...
	feeInfoRange = exccjson.FeeInfoRange{
	...
	return &exccjson.TxFeeInfoResult{
		FeeInfoMempool: *feeInfoMempool,
		FeeInfoBlocks:  feeInfoBlocks,
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.ValidateAddressCmd)
	result := exccjson.ValidateAddressChainResult{}
	addr, err := exccutil.DecodeAddress(c.Address)
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*exccjson.VerifyMessageCmd)
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := map[string]exccjson.VersionResult{
		"exccdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, exccjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *exccjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(exccjson.Error); ok &&
			jerr.Code == exccjson.ErrUnregisteredMethod {
			parsedCmd.err = exccjson.ErrRPCMethodNotFound
			return &parsedCmd
		}
...
func createMarshalledReply(rpcVersion string, id interface{}, result interface{}, replyErr error) ([]byte, error) {
	...
	if replyErr != nil {
		if jErr, ok := replyErr.(*exccjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return exccjson.MarshalResponse(rpcVersion, id, result, jsonErr)
}

// processRequest determines the incoming request type (single or batched),
// parses it and returns a marshalled response.
func (s *rpcServer) processRequest(request *exccjson.Request, isAdmin bool, closeChan <-chan struct{}) []byte {
...
func (s *rpcServer) jsonRPCRead(w http.ResponseWriter, r *http.Request, isAdmin bool) {
	...
	if !batchedRequest {
		...
		if err != nil {
			...
			resp, err = exccjson.MarshalResponse("1.0", nil, nil, jsonErr)
			if err != nil {
	...
	if batchedRequest {
		...
		if err != nil {
			...
			resp, err = exccjson.MarshalResponse("2.0", nil, nil, jsonErr)
			if err != nil {
				rpcsLog.Errorf("Failed to create reply: %v", err)
			}
			...
			if len(batchedRequests) > 0 {
				...
				for _, entry := range batchedRequests {
					...
					if err != nil {
						...
						resp, err = exccjson.MarshalResponse("", nil, nil, jsonErr)
						if err != nil {
							rpcsLog.Errorf("Failed to create reply: %v", err)
						}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="exccd RPC"`)
