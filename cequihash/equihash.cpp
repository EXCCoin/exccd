...
void Equihash<N,K>::InitialiseState(eh_HashState& base_state, const void* input, uint32_t input_len, int64_t nonce)
{
    blake2b_param P;
    ...
    *(uint32_t *) (P.personal + 8) = htole32(N);
    *(uint32_t *) (P.personal + 12) = htole32(K);

    blake2b_init_param(&base_state, &P);

    if (input && input_len) {
        blake2b_update(&base_state, static_cast<const uint8_t *>(input), input_len);
    }

    if (nonce >= 0) {
        uint32_t expandedNonce[8] = {0};
        expandedNonce[0] = htole32(static_cast<uint32_t >(nonce));

        blake2b_update(&base_state, reinterpret_cast<const uint8_t *>(&expandedNonce), sizeof(expandedNonce));
    }
}
...
{
    static_assert(sizeof(eh_index) == 4, "Indexes must be of exactly 4 bytes");
...
{
    // Truncate to 8 bits
    static_assert(sizeof(eh_trunc) == 1, "eh_trunc must be exctly 1 byte");
...
{
    static_assert(W <= WIDTH, "W must be less than WIDTH");
                                   ...
                                   const std::function<bool(EhSolverCancelCheck)> cancelled)
{
    ...
    for (std::shared_ptr<eh_trunc> partialSoln : partialSolns) {
        ...
        size_t lenIndices;
        unsigned char tmpHash[HashOutput];
        //TODO: (siy) is it causes any issues???
        //std::vector<boost::optional<std::vector<FullStepRow<FinalFullWidth>>>> X;
        std::vector<tl::optional<std::vector<FullStepRow<FinalFullWidth>>>> X;
        X.reserve(K+1);
        ...
        for (eh_index i = 0; i < soln_size; i++) {
            ...
            tl::optional<std::vector<FullStepRow<FinalFullWidth>>> ic = icv;
...
template<unsigned int N, unsigned int K>
ValidationResult Equihash<N,K>::ValidateSolution(const eh_HashState &base_state, std::vector<unsigned char> soln)
{
    if (soln.size() != SolutionWidth) {
        return POW_INVALID_SOLUTION_LEN;
    }
    ...
    assert(X.size() == 1);
    return X[0].IsZero(hashLen) ? POW_OK : POW_NON_ZERO_XOR;
}

// Explicit instantiations for Equihash<144,5>
template void Equihash<144,5>::InitialiseState(eh_HashState& base_state, const void* input, uint32_t input_len, int64_t nonce);
...
template ValidationResult Equihash<144,5>::ValidateSolution(const eh_HashState &base_state, std::vector<unsigned char> soln);

// Explicit instantiations for Equihash<200,9>
template void Equihash<200,9>::InitialiseState(eh_HashState& base_state, const void* input, uint32_t input_len, int64_t nonce);
...
template ValidationResult Equihash<200,9>::ValidateSolution(const eh_HashState &base_state, std::vector<unsigned char> soln);

// Explicit instantiations for Equihash<96,5>
template void Equihash<96,5>::InitialiseState(eh_HashState& base_state, const void* input, uint32_t input_len, int64_t nonce);
...
template ValidationResult Equihash<96,5>::ValidateSolution(const eh_HashState &base_state, std::vector<unsigned char> soln);

// Explicit instantiations for Equihash<48,5>
template void Equihash<48,5>::InitialiseState(eh_HashState& base_state, const void* input, uint32_t input_len, int64_t nonce);
...
template ValidationResult Equihash<48,5>::ValidateSolution(const eh_HashState &base_state, std::vector<unsigned char> soln);
