...
// newAddressPubKey returns a new exccutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) exccutil.Address {
	pubkey, err := chainec.Secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := exccutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new exccutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) exccutil.Address {
	addr, err := exccutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
		secp)
	if err != nil {
...
// newAddressScriptHash returns a new exccutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) exccutil.Address {
	addr, err := exccutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the exccutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the exccutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := exccutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := exccutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := exccutil.NewAddressSecpPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := exccutil.NewAddressSecpPubKey(decodeHex("03b0bd"+
	...
	tests := []struct {
		...
		{(*exccutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*exccutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*exccutil.AddressSecpPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := exccutil.NewAddressSecpPubKey(decodeHex("02192d7"+
	...
	tests := []struct {
		keys      []*exccutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*exccutil.AddressSecpPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
		...
		},
		{
			...
			[]*exccutil.AddressSecpPubKey{
				p2pkUncompressedMain.(*exccutil.AddressSecpPubKey),
			},
			1,
