...
// newAddressPubKey returns a new excutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) excutil.Address {
	pubkey, err := chainec.Secp256k1.ParsePubKey(serializedPubKey)
	if err != nil {
		panic("invalid public key in test source")
	}
	addr, err := excutil.NewAddressSecpPubKeyCompressed(pubkey,
...
// newAddressPubKeyHash returns a new excutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) excutil.Address {
	addr, err := excutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams,
		secp)
	if err != nil {
...
// newAddressScriptHash returns a new excutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) excutil.Address {
	addr, err := excutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the excutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the excutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := excutil.NewAddressPubKeyHash(decodeHex("e34cce70c863"+
	...
	p2shMain, _ := excutil.NewAddressScriptHashFromHash(decodeHex("e8c300"+
	...
	p2pkCompressedMain, err := excutil.NewAddressSecpPubKey(decodeHex("02192d74"+
	...
	p2pkCompressed2Main, err := excutil.NewAddressSecpPubKey(decodeHex("03b0bd"+
	...
	tests := []struct {
		...
		{(*excutil.AddressPubKeyHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*excutil.AddressScriptHash)(nil), "", txscript.ErrUnsupportedAddress},
		{(*excutil.AddressSecpPubKey)(nil), "", txscript.ErrUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	//  mainnet p2pk 13CG6SJ3yHUXo4Cr2RY4THLLJrNFuG3gUg
	p2pkCompressedMain, err := excutil.NewAddressSecpPubKey(decodeHex("02192d7"+
	...
	tests := []struct {
		keys      []*excutil.AddressSecpPubKey
		nrequired int
		expected  string
		err       error
	}{
