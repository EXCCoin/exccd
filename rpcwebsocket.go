...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *exccutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *exccutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *exccutil.Tx, isNew bool) {
...
func (f *wsClientFilter) addAddress(a exccutil.Address) {
	switch a := a.(type) {
	case *exccutil.AddressPubKeyHash:
		f.pubKeyHashes[*a.Hash160()] = struct{}{}
		return
	case *exccutil.AddressScriptHash:
...
func (f *wsClientFilter) addAddressStr(s string) {
	a, err := exccutil.DecodeAddress(s)
...
func (f *wsClientFilter) existsAddress(a exccutil.Address) bool {
	switch a := a.(type) {
	case *exccutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected exccutil.Block
type notificationBlockDisconnected exccutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *exccutil.Tx, clients map[chan struct{}]*wsClient) map[chan struct{}]struct{} {
...
func (m *wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient, block *exccutil.Block) {
	...
	ntfn := exccjson.BlockConnectedNtfn{
	...
	for quitChan, client := range clients {
		...
		marshalledJSON, err := exccjson.MarshalCmd("1.0", nil, &ntfn)
		if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *exccutil.Block) {
	...
	ntfn := exccjson.BlockDisconnectedNtfn{
		Header: hex.EncodeToString(headerBytes),
	}
	marshalledJSON, err := exccjson.MarshalCmd("1.0", nil, &ntfn)
	if err != nil {
...
func (m *wsNotificationManager) notifyReorganization(clients map[chan struct{}]*wsClient, rd *blockchain.ReorganizationNtfnsData) {
	...
	// Notify interested websocket clients about the disconnected block.
	ntfn := exccjson.NewReorganizationNtfn(rd.OldHash.String(),
		int32(rd.OldHeight),
		rd.NewHash.String(),
		int32(rd.NewHeight))
	marshalledJSON, err := exccjson.MarshalCmd("1.0", nil, ntfn)
...
func (*wsNotificationManager) notifyWinningTickets(
	...
	ntfn := exccjson.NewWinningTicketsNtfn(wtnd.BlockHash.String(),
...
func (*wsNotificationManager) notifySpentAndMissedTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := exccjson.NewSpentAndMissedTicketsNtfn(tnd.Hash.String(),
...
func (*wsNotificationManager) notifyNewTickets(clients map[chan struct{}]*wsClient, tnd *blockchain.TicketNotificationsData) {
	...
	// Notify interested websocket clients about the connected block.
	ntfn := exccjson.NewNewTicketsNtfn(tnd.Hash.String(), int32(tnd.Height),
...
func (*wsNotificationManager) notifyStakeDifficulty(clients map[chan struct{}]*wsClient, sdnd *StakeDifficultyNtfnData) {
	// Notify interested websocket clients about the connected block.
	ntfn := exccjson.NewStakeDifficultyNtfn(sdnd.BlockHash.String(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *exccutil.Tx) {
	...
	ntfn := exccjson.NewTxAcceptedNtfn(txHashStr,
		exccutil.Amount(amount).ToCoin())
	marshalledJSON, err := exccjson.MarshalCmd("1.0", nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *exccjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = exccjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = exccjson.MarshalCmd("1.0", nil,
				verboseNtfn)
			if err != nil {
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *exccutil.Tx,
	...
	if len(clientsToNotify) != 0 {
		n := exccjson.NewRelevantTxAcceptedNtfn(txHexString(msgTx))
		marshalled, err := exccjson.MarshalCmd("1.0", nil, n)
		if err != nil {
	...
	for {
		...
		if !batchedRequest {
			...
			if req.Method == "" || req.Params == nil {
				jsonErr := &exccjson.RPCError{
					Code:    exccjson.ErrRPCInvalidRequest.Code,
			...
			switch authCmd, ok := cmd.cmd.(*exccjson.AuthenticateCmd); {
			...
			if !c.isAdmin {
				if _, ok := rpcLimited[req.Method]; !ok {
					jsonErr := &exccjson.RPCError{
						Code:    exccjson.ErrRPCInvalidParams.Code,
		...
		if batchedRequest {
			...
			if err != nil {
				...
				reply, err = exccjson.MarshalResponse("2.0", nil, nil, jsonErr)
				if err != nil {
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*exccjson.HelpCmd)
	if !ok {
		return nil, exccjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*exccjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &exccjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*exccjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, exccjson.ErrRPCInternal
	}
...
// string slice.
func rescanBlock(filter *wsClientFilter, block *exccutil.Block) []string {
...
func handleRescan(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*exccjson.RescanCmd)
	if !ok {
		return nil, exccjson.ErrRPCInternal
	}
	...
	if filter == nil {
		return nil, &exccjson.RPCError{
			Code:    exccjson.ErrRPCMisc,
			Message: "Transaction filter must be loaded before rescanning",
		}
	}

	blockHashes, err := exccjson.DecodeConcatenatedHashes(cmd.BlockHashes)
	if err != nil {
		return nil, err
	}

	discoveredData := make([]exccjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &exccjson.RPCError{
				Code:    exccjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, exccjson.RescannedBlock{
	...
	return &exccjson.RescanResult{DiscoveredData: discoveredData}, nil
}
