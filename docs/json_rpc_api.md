...
exccd provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between exccd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, exccd intentionally splits the wallet and chain
...
The exccd RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
exccd is `wss://your_ip_or_domain:9109/ws`.
...
to a exccd RPC server:

* **rpcuser** is the full-access username configured for the exccd RPC server
* **rpcpass** is the full-access password configured for the exccd RPC server
* **rpclimituser** is the limited username configured for the exccd RPC server
* **rpclimitpass** is the limited password configured for the exccd RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the exccd
  server is configured with.  It is automatically generated by exccd and placed
  in the exccd home directory (which is typically `%LOCALAPPDATA%\Exccd` on
  Windows and `~/.exccd` on POSIX-like OSes)

**NOTE:** As mentioned above, exccd is secure by default which means the RPC
...
The exccd RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
exccd comes with a separate utility named `exccctl` which can be used to issue
these RPC commands via HTTP POST requests to exccd after configuring it with the
...
API compatible with the original exccd client.
...
|24|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since exccd does not have the wallet integrated to provide payment addresses, exccd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/exccd:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since exccd does not have the wallet integrated to provide payment addresses, exccd must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|exccd does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown exccd.|
|Returns|`"exccd stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For exccd this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|exccd currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for exccd.|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this exccd instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
exccd uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll exccd for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a exccd instance via Websockets, issues [getblockcount](#getblockcount) to
...
The following is an example Go application which uses the
[rpcclient](https://github.com/EXCCoin/exccd/tree/master/rpcclient) package to connect with
a exccd instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/EXCCoin/exccd/tree/master/rpcclient) package to connect with
a exccd instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a exccd instance,
...
// generated by exccd when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/exccd/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The exccd generated certificate acts as
