...
type Indexer interface {
	...
	ConnectBlock(dbTx database.Tx, block, parent *exccutil.Block, view *blockchain.UtxoViewpoint) error

	// DisconnectBlock is invoked when the index manager is notified that a
	// block has been disconnected from the main chain.
	DisconnectBlock(dbTx database.Tx, block, parent *exccutil.Block, view *blockchain.UtxoViewpoint) error
}
...
// be considered valid.
func approvesParent(block *exccutil.Block) bool {
	return exccutil.IsFlagSet16(block.MsgBlock().Header.VoteBits,
		exccutil.BlockValid)
}
