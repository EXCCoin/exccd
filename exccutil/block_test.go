...
func TestBlock(t *testing.T) {
	b := exccutil.NewBlock(&Block100000)
	...
	wantHashStr := "7d9a8e69628f913086b35130524769b0e2eaff0fcdac97345e28ef7055a7767e"
...
func TestNewBlockFromBytes(t *testing.T) {
	...
	b, err := exccutil.NewBlockFromBytes(block100000Bytes)
	if err != nil {
...
func TestNewBlockFromBlockAndBytes(t *testing.T) {
	...
	// Create a new block from the serialized bytes.
	b := exccutil.NewBlockFromBlockAndBytes(&Block100000, block100000Bytes)
...
func TestBlockErrors(t *testing.T) {
	...
	testErr := exccutil.OutOfRangeError(wantErr)
	...
	_, err = exccutil.NewBlockFromBytes(shortBytes)
	if err != io.EOF {
	...
	_, err = b.TxHash(-1)
	if _, ok := err.(exccutil.OutOfRangeError); !ok {
		t.Errorf("TxHash: wrong error - got: %v <%T>, "+
			"want: <%T>", err, err, exccutil.OutOfRangeError(""))
	}
