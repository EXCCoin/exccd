...
const (
	...
	unusableFlags = exccjson.UFWebsocketOnly | exccjson.UFNotification
)
...
var (
	...
	defaultWalletCertFile  = filepath.Join(exccwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := exccjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := exccjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := exccjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&exccjson.UFWalletOnly != 0 {
...
// config defines the configuration options for exccexccctl.
//
...
func createDefaultConfigFile(destinationPath string) error {
	...
	defer exccdConfigFile.Close()
	content, err := ioutil.ReadAll(exccdConfigFile)
	if err != nil {
		return err
	}
