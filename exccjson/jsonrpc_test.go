...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if exccjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *exccjson.RPCError {
				return exccjson.NewRPCError(exccjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := exccjson.MarshalResponse("1.0", testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := exccjson.NewRequest("1.0", nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := exccjson.Error{Code: exccjson.ErrInvalidType}
	_, err = exccjson.MarshalResponse("", make(chan int), nil, nil)
	if jerr, ok := err.(exccjson.Error); !ok || jerr.Code != wantErr.Code {
	...
	// can't be marshalled.
	_, err = exccjson.MarshalResponse("1.0", 1, make(chan int), nil)
